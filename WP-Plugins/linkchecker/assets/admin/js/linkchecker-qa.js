		...
		var events = {
			...
			handleDraftWarning: function(cvalue) {
				...
				'<div id="broken-link-error">\
					...
					<p>You have broken links in this post. Click on the links below to scroll down to each broken link. Publishing live will be disabled until these urls are resolved.</p>\
				...
				if(res.length > 0) {
					...
					if(saveDraft) {
						$('#broken-link-error').append(
							'<p>Your changes have not been saved. Click the button below to save your draft</p>\
							<button id="save-draft-no-warning" class="button">Save Draft</button>'
						);
					}
					if(superAdmin){
						$('#broken-link-error').append(
